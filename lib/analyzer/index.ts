...
import { searchPublishedDate } from "./llm-client";
import { persistAnalysis } from "./db-persistence";
...
program
  ...
  .option(
    "--published-date <date>",
    "Дата публикации (переопределяет веб-поиск)",
  )
  .action(
    async (
      pdfPath: string,
      ...
      try {
        ...
        console.log(
          `    Второстепенные: ${result.characters.secondary.length}`,
        );
        console.log(`    Эпизодические:  ${result.characters.minor.length}`);
        console.log(
          `  Событий:          ${result.plotSummary.keyEvents.length}`,
        );
